...
pro = ts.pro_api('3a5d8df1d0f8bbbe5bd54664417eaf8b60f4bfea4a8551f2926f8019')
plotly.offline.init_notebook_mode(connected=True)

def data_feed(date): # 提取数据
    # 提取50ETF合约基础信息
...
def data_clear(df): # 数据清洗
    ...
    df_p = df_p.rename(columns={'c_p': 'c', 'ts_code_p': 'ts_code',
                         'call_put_p':'call_put'})
    df_c = pd.concat([df_c, df_p], axis=0)
    #print(df_c)
    return df_c        
...
def im_surface(df,date): # 波动率曲面作图
    # df = plot_df()
    df = fitting(df)
    ...
    plotly.offline.plot(fig,filename=f'/Users/yangtianyu/Desktop/surface_{date}.html',auto_open=False)

def smile_plot(df,date): # 波动率微笑作图
    # df = plot_df()
    df = data_pivot(df)
    df = df.set_index('k')
    ...
    plotly.offline.plot(fig,filename=f'/Users/yangtianyu/Desktop/smile_{date}.html',auto_open=False)

def main(date):
    date = date # 可任意更改日期
    df = data_feed(date) # 提取数据
    df = data_clear(df) # 数据清洗
    df = cal_iv(df) # 计算隐含波动率
    data_pivot(df) # 数据透视表
    # smile_plot(df,date) # 波动率微笑
    im_surface(df,date) # 波动率曲面
    return df

def get_trade_date(start, end, step=1, format="%Y-%m-%d"):
    strptime, strftime = datetime.datetime.strptime, datetime.datetime.strftime
    days = (strptime(end, format) - strptime(start, format)).days
    sleep_time = days/60
    date_list = [strftime(strptime(start, format) + datetime.timedelta(i), format="%Y%m%d") for i in range(0, days, step)]
    date = []
    for i in date_list:
        df_cal = pro.trade_cal(exchange='SSE', cal_date=i, fields = 'cal_date,is_open,pretrade_date')
        if df_cal.iloc[0,1] == 0:
            i = df_cal.iloc[0,2]
        else:pass
        date.append(i)
...
if __name__ == '__main__':
    main('20220715')
    # date_list = get_trade_date('2022-05-01','2022-07-02')
